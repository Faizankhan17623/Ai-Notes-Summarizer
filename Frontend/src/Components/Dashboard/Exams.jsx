...
const downloadCalendar = (exam, schedule) => {
    const esc = (value) => String(value || '').replace(/[\\,;]/g, (m) => `\\${m}`).replace(/\r?\n/g, '\\n')
    const events = schedule.map((day) => {
        const start = `${day.date.replaceAll('-', '')}T090000Z`
        const end = `${day.date.replaceAll('-', '')}T${String(9 + Math.min(8, Math.ceil((day.minutes || 30) / 60))).padStart(2, '0')}0000Z`
        return ['BEGIN:VEVENT', `UID:notewise-${exam._id}-${day.date}@notewise`, `DTSTAMP:${start}`, `DTSTART:${start}`, `DTEND:${end}`, `SUMMARY:${esc(`${exam.title}: ${day.task}`)}`, `DESCRIPTION:${esc(`${day.minutes || 30} minutes of exam preparation`)}`, 'END:VEVENT'].join('\r\n')
    })
    const ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Notewise//Exam Preparation//EN', 'CALSCALE:GREGORIAN', ...events, 'END:VCALENDAR'].join('\r\n')
    const blob = new Blob([ics], { type: 'text/calendar;charset=utf-8' }); const href = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = href; a.download = `${exam.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-calendar.ics`; a.click(); URL.revokeObjectURL(href)
}
            ...
            <div className="max-w-5xl mx-auto px-4 py-8">
                ...
                {calendarExam && <div className="border border-violet-400/40 bg-surface rounded-lg p-5 mb-5">
                    ...
                    <div className="flex flex-wrap gap-3 items-end">
                        ...
                        {schedule.length > 0 && <button type="button" onClick={() => downloadCalendar(calendarExam, schedule)} className="border border-border-soft text-richblack-200 rounded px-3 py-2 text-sm">Export .ics</button>}
                    </div>
