...
const ApiKeySection = ({ isPaidPlan }) => {
    ...
    return (
        <div className="border border-border-soft bg-surface rounded-lg p-6 space-y-3">
            ...
            <p className="text-richblack-400 text-sm">
                Call <code className="bg-surface-hover px-1.5 py-0.5 rounded text-xs font-mono">POST /external/summarize</code> with an{' '}
                <code className="bg-surface-hover px-1.5 py-0.5 rounded text-xs font-mono">x-api-key</code> header. This key only works on that one endpoint.
            </p>

            {freshKey && (
                <div className="bg-surface-hover border border-yellow-50 rounded-md p-3 flex items-center justify-between gap-3">
            ...
            {freshKey && <p className="text-warn text-xs">Copy it now — it will not be shown again.</p>}
