...
const StatCard = ({ label, value, icon: Icon, tone }) => (
    <div className="border border-border-soft bg-surface rounded-lg p-4 flex items-center gap-3">
        <span className={`w-9 h-9 rounded-md flex items-center justify-center shrink-0 ${tone === 'danger' ? 'bg-danger-soft/10 text-danger-soft' : 'bg-yellow-50/10 text-yellow-50'}`}>
            <Icon size={14} />
        </span>
        <div>
...
const Payments = () => {
    ...
    const stats = useMemo(() => {
        ...
        return { totalRevenue, paidCount: paid.length, failedCount: failed.length }
    }, [payments])
        ...
        <div className="px-6 md:px-10 py-10">
            ...
            {payments.length > 0 && (
                <div className="grid sm:grid-cols-3 gap-4 mb-6">
                    <StatCard label="Total revenue" value={`₹${stats.totalRevenue}`} icon={FaRupeeSign} />
                    <StatCard label="Successful payments" value={stats.paidCount} icon={FaCheckCircle} />
                    <StatCard label="Failed payments" value={stats.failedCount} icon={FaTimesCircle} tone="danger" />
                </div>
            )}
